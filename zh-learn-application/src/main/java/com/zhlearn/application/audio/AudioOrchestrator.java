...
public class AudioOrchestrator {
    ...
    public List<PronunciationCandidate> candidatesFor(Hanzi word, Pinyin pinyin) {
        ...
        for (AudioProvider provider : audioProviders) {
            List<AudioProvider.PronunciationDescription> descriptions = provider.getPronunciationsWithDescriptions(word, pinyin);
            for (AudioProvider.PronunciationDescription desc : descriptions) {
                if (desc.path() == null) continue;
